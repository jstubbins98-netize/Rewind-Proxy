"""
Raspberry Pi 5 ethernet connection wizard for Rewind-Proxy.

When running on a Pi 5 this module:
  1. Detects the Pi 5 hardware
  2. Finds the wired ethernet interface
...
_state: dict = {
    ...
    "eth_ip":           None,   # IP assigned to the Pi on that interface
    "proxy_port":       8080,
...
def find_ethernet_interface() -> str | None:
    """
    Find the first wired ethernet interface.
    Prefers 'eth0', then names matching eth* or en* that have type==1 (Ethernet).
    """
    ...
    for iface in ifaces:
        ...
        try:
            with open(type_path) as f:
                if f.read().strip() == "1":   # ARPHRD_ETHER = 1
    ...
    # Put eth0 first, then eth*, then en*, then rest
    def sort_key(name):
        if name == "eth0":
            return (0, name)
...
def ensure_dnsmasq_installed() -> bool:
    ...
    install it via apt-get.  Returns True if dnsmasq is available after the attempt.
    """
    ...
    logger.info("dnsmasq not found — installing via apt-get (this may take a moment)…")
    try:
        result = subprocess.run(
        ...
        if result.returncode == 0 and shutil.which("dnsmasq"):
            ...
            # apt-get may have left a system dnsmasq service running — we will
            # stop it below in start_dnsmasq before launching our own instance
            return True
        else:
            logger.warning(
                "apt-get install dnsmasq failed (rc=%d): %s",
                result.returncode,
                result.stderr.decode(errors="replace"),
            )
    except Exception as exc:
        logger.warning("apt-get install dnsmasq exception: %s", exc)
...
def start_dnsmasq(iface: str) -> bool:
    ...
    Strategy to avoid port conflicts on Pi OS Bookworm:
      • --port=0 disables dnsmasq's DNS listener entirely (no port 53 needed)
        so systemd-resolved can stay running without conflict.
      • We stop the system dnsmasq service (if any) before launching ours so
        port 67 (DHCP) is free.
    ...
    # Stop the system-managed dnsmasq service so it does not hold port 67
    _run(["systemctl", "stop", "dnsmasq"])
    _run(["systemctl", "disable", "dnsmasq"])

    # Kill any leftover dnsmasq processes on this interface
    _run(["pkill", "-f", f"dnsmasq.*{iface}"])
...
def enable_ip_forwarding() -> None:
    """Enable IPv4 forwarding so the Pi can route traffic to the internet."""
    try:
...
def _monitor_loop(iface: str, proxy_port: int) -> None:
    ...
    logger.info("Pi 5 ethernet monitor started on interface %s (root=%s)", iface, is_root)
...
def start(proxy_port: int = 8080) -> bool:
    """
    Detect Pi 5, find ethernet interface, start the monitor thread.
    Returns True if the Pi 5 wizard was activated, False otherwise.
    """
    global _monitor_thread

    if not is_raspberry_pi_5():
        with _lock:
            _state["status"] = S_NOT_PI
        logger.debug("Not a Raspberry Pi 5 — skipping ethernet wizard")
        return False
    ...
    if not iface:
        with _lock:
            ...
            _state["error"]  = "No wired ethernet interface found."
        logger.warning("Pi 5 detected but no ethernet interface found")
        return False

    logger.info("Raspberry Pi 5 detected — starting ethernet wizard on %s", iface)
