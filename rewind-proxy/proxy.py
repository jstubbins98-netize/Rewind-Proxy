...
def _build_headless_status() -> bytes:
    ...
    try:
        ...
        if status == pi_setup.S_CONNECTED:
            ...
            pi_line = f"\nPi ethernet : old computer connected at {old_ip}"
        elif status == pi_setup.S_WAITING_DEVICE:
            pi_line = "\nPi ethernet : interface ready — waiting for old computer"
        elif status == pi_setup.S_WAITING_CABLE:
            pi_line = "\nPi ethernet : waiting for ethernet cable"
        elif status not in (pi_setup.S_NOT_PI, ""):
            pi_line = f"\nPi ethernet : {status}"
    except Exception:
        pass
