...
# Directory containing bundled audio files (dialup.mp3, welcome.mp3)
...
#rp-titlebar-icon {{ font-size: 14px; }}
#rp-body {{
...
/* ── Main content (hidden until splash finishes) ──────────── */
#rp-main {{ display: none; }}
</style>
</head>
<body bgcolor="#FFFFFF">

<!-- ═══════════════════════════════════════════════════════════
     SPLASH SCREEN  (hidden by JS after audio sequence completes)
     Falls back gracefully: if JS/audio not supported the main
     content is shown immediately via the noscript block below.
     ═══════════════════════════════════════════════════════════ -->
<div id="rp-splash">
  <div id="rp-dialog">
    <div id="rp-titlebar">
      <span id="rp-titlebar-icon">&#x1F4BB;</span>
      <span id="rp-titlebar-text">Rewind-Proxy &mdash; Connecting...</span>
    </div>
    <div id="rp-body">
      <div id="rp-modem">+---------------------------------------+
| PWR  CD   RD   TD   MR   DTR  RI  AA |
| [*]  [ ]  [ ]  [ ]  [*]  [*]  [ ] [ ]|
|                                       |
|   H A Y E S   C O M P A T I B L E   |
|          56K  FAX/MODEM               |
+---------------------------------------+</div>
      <div id="rp-status">Initialising modem...</div>
      <div id="rp-substatus"></div>
      <div id="rp-track"><div id="rp-bar"></div></div>
      <button id="rp-btn" onclick="rpStart()">&#x260F;&nbsp; Click to Connect</button>
    </div>
  </div>
</div>

<!-- Audio elements — preloaded, never autoplay via attribute -->
<audio id="rp-snd-dialup"  src="/audio/dialup.mp3"  preload="auto"></audio>
<audio id="rp-snd-welcome" src="/audio/welcome.mp3" preload="auto"></audio>

<script>
(function () {{
  // ── All element lookups happen inside init() so the DOM is
  // ── guaranteed to exist before we touch it.
  function init() {{
    var splash     = document.getElementById('rp-splash');
    var tbtext     = document.getElementById('rp-titlebar-text');
    var statusEl   = document.getElementById('rp-status');
    var substatus  = document.getElementById('rp-substatus');
    var bar        = document.getElementById('rp-bar');
    var btn        = document.getElementById('rp-btn');
    var mainEl     = document.getElementById('rp-main');
    var sndDialup  = document.getElementById('rp-snd-dialup');
    var sndWelcome = document.getElementById('rp-snd-welcome');

    // Only show the splash once per browser session.
    // Navigating back to / (e.g. clicking Home on an archived page) skips it.
    try {{
      if (sessionStorage.getItem('rp-booted')) {{
      ...
      // sessionStorage unavailable (private mode, old browser) — show splash anyway
    }}

    var dotCount = 0, dotTimer = null, dismissed = false;
    ...
    function animateDots() {{
      ...
      substatus.textContent = new Array(dotCount + 1).join('\u25a0 ');
      dotTimer = setTimeout(animateDots, 350);
    }}
    ...
    function showMain() {{
      ...
      stopDots();
      setProgress(100);
      tbtext.textContent   = 'Rewind-Proxy \u2014 Ready';
    ...
    function playWelcome() {{
      stopDots();
      tbtext.textContent    = 'Rewind-Proxy \u2014 Welcome!';
      statusEl.textContent  = 'Connected!';
      substatus.textContent = '';
      setProgress(80);
      sndWelcome.onended = showMain;
      sndWelcome.play().catch(showMain);
      setTimeout(showMain, 12000); // safety
    }}

    function rpStart() {{
      btn.style.display    = 'none';
      tbtext.textContent   = 'Rewind-Proxy \u2014 Dialing...';
      statusEl.textContent = 'Dialing...';
      setProgress(10);
      animateDots();

      sndDialup.onended = function () {{
        setProgress(60);
        playWelcome();
      }};

      var p = sndDialup.play();
      if (p !== undefined) {{
        p.then(function () {{
          setProgress(20);
        }}).catch(function () {{
          stopDots();
          tbtext.textContent    = 'Rewind-Proxy \u2014 Click to Connect';
          statusEl.textContent  = 'Autoplay blocked';
          substatus.textContent = 'Click the button below';
          setProgress(0);
    ...
    // Expose for the button onclick attribute
    window.rpStart = rpStart;

    // Clicking anywhere on the splash (while blocked) also starts it
    splash.addEventListener('click', function () {{
      if (btn.style.display !== 'none') rpStart();
    }});

    rpStart();

    // Absolute safety net — reveal main after 40 s no matter what
    setTimeout(showMain, 40000);
  }}

  // Run init() only after the full DOM is parsed
...
<noscript>
<style>#rp-splash{{display:none}} #rp-main{{display:block!important}}</style>
</noscript>

<!-- ═══════════════════════════════════════════════════════════
     MAIN CONTENT  (revealed after the splash audio sequence)
     ═══════════════════════════════════════════════════════════ -->
<div id="rp-main">
<h1>Rewind-Proxy</h1>
...
class RewindProxyHandler(BaseHTTPRequestHandler):
    ...
    def _handle_direct(self, method: str, path: str):
        ...
        # /audio/<file> — serve bundled audio files
    ...
    def _handle_proxy(self, method: str, url: str):
        ...
        if (_pu.path.startswith("/r") or _pu.path.startswith("/go")
                or _pu.path.startswith("/setup") or _pu.path.startswith("/audio/")):
    ...
    def _serve_audio_file(self, path: str):
        """Serve a bundled audio file from the rewind-proxy/audio/ directory."""
        # path is like /audio/dialup.mp3 — strip the /audio/ prefix
        filename = os.path.basename(urllib.parse.urlparse(path).path)
        filepath = os.path.join(_AUDIO_DIR, filename)

        # Safety: only serve files that actually live inside _AUDIO_DIR
        real_path = os.path.realpath(filepath)
        if not real_path.startswith(os.path.realpath(_AUDIO_DIR) + os.sep):
            self._send_response(403, "Forbidden", b"text/plain", b"Forbidden")
            return

        if not os.path.isfile(real_path):
            self._send_response(404, "Not Found", b"text/plain",
                                f"Audio file '{filename}' not found.".encode())
            return
        ...
        with open(real_path, "rb") as f:
            data = f.read()
        self._send_response(200, "OK", mime.encode(), data,
