...
def _build_pi_section() -> str:
    """
    Return an HTML string for the Linux ethernet wizard section.
    Returns an empty string when not running on Linux.
    """
    st = pi_setup.get_status()
    status = st["status"]

    if status == pi_setup.S_NOT_LINUX:
        return ""   # Not Linux — hide this section entirely
...
<tr {step_style[0]}><td width="30"><b>1</b></td>
  <td>Plug the ethernet cable from your old computer into the selected Linux computer's ethernet port.</td></tr>
<tr {step_style[1]}><td><b>2</b></td>
  <td>Rewind-Proxy configures the port and waits for your old computer to appear.</td></tr>
<tr {step_style[2]}><td><b>3</b></td>
  <td>Configure your old computer's browser proxy settings &mdash;
      the Linux host will fetch all pages from the Wayback Machine on its behalf.</td></tr>
</table>
"""
...
<p>Take a standard ethernet cable and plug one end into your old computer
 and the other into the selected Linux ethernet port
...
<p><small>
  ...
  cable usually works fine on modern ethernet hardware with auto-MDI/MDI-X.
</small></p>
"""
        ...
        if is_root:
            ...
            body = f"""
<p><b>&#9654; Step 2: The Linux host is ready. Now set up your old computer.</b></p>
<p>The ethernet port (<tt>{iface}</tt>) is configured as
...
"""
            ...
            body = f"""
<p><b>&#9654; Step 2: Run the following commands on the Linux host to configure the ethernet port.</b></p>
<p>Rewind-Proxy is not running as root, so the network must be set up manually.
Open a terminal on the Linux host and run:</p>
<ol>{cmd_list}</ol>
...
<p>Then browse to any <tt>http://</tt> address &mdash; the Linux host will fetch
...
"""
    ...
    # Do not auto-refresh this page. A meta refresh resets the scroll position
    # in old browsers every few seconds and makes the instructions unusable.
    refresh_link = ""
    if status in (pi_setup.S_WAITING_CABLE, pi_setup.S_CONFIGURING,
                  pi_setup.S_WAITING_DEVICE, pi_setup.S_DETECTING):
        refresh_link = '<p><a href="/">Refresh connection status</a></p>'

    return f"""
<hr>
<h2>Linux Ethernet &mdash; Old Computer Connection Wizard</h2>
{steps_html}
{body}
{refresh_link}
"""
...
<h2>Help</h2>
<ul>
  <li><a href="/setup">&#x1F4BB; How to connect a real old computer through a Linux host</a></li>
</ul>
<hr>
...
def _build_setup_page() -> bytes:
    """Return the HTML setup page for a Linux host and old computer."""
...
on a <b>real old computer</b> using a <b>Linux computer</b> as the go-between.</p>

<hr>
<h2>What you need</h2>
<ul>
  <li>A Linux computer (including Raspberry Pi 5) running Rewind-Proxy</li>
  <li>An ethernet cable</li>
  <li>The old computer you want to browse from</li>
  <li>A modern computer or phone to control the Linux host (optional but helpful)</li>
</ul>

<hr>
<h2>Step 1 &mdash; Connect the old computer to the Linux host</h2>
<p>Plug one end of the ethernet cable into the <b>old computer</b> and the other end
into the <b>selected Linux ethernet port</b>.</p>
<p>Rewind-Proxy will automatically create a private network over that cable.
If the home page shows a green &ldquo;Old computer connected&rdquo;
banner, the link is already up.</p>

<hr>
<h2>Step 2 &mdash; Give the old computer a network address</h2>
<p>If the Linux host is running dnsmasq (Rewind-Proxy tries to install and start it automatically when run as root),
...
Enter <b>the Linux host&rsquo;s private ethernet IP address</b> as the HTTP proxy:</p>
<table border="1" cellpadding="4" cellspacing="0">
<tr><th align="left">Setting</th><th align="left">Value</th></tr>
...
<dt><b>The ethernet wizard reports that no safe interface was found</b></dt>
<dd>Rewind-Proxy will not overwrite the interface carrying the Linux host&rsquo;s
default internet route. Connect the host to the internet with Wi-Fi or another
adapter, then use a separate wired port for the old computer. If there are
multiple wired ports, restart with <tt>--interface NAME</tt>.</dd>
</dl>
...
def _build_headless_status() -> bytes:
    """
    Minimal plain-text status page served at / in headless mode.
    No web UI — just enough for a browser or monitoring tool to confirm
    the proxy is alive and get the connection details.
    """
    import datetime
    proxy_host = os.environ.get("REWIND_PROXY_HOST", "localhost")
    proxy_port = os.environ.get("REWIND_PROXY_PORT", "8080")
    ...
    try:
        ...
        if status == pi_setup.S_CONNECTED:
            old_ip = st.get("old_computer_ip", "unknown")
            pi_line = f"\nLinux ethernet: old computer connected at {old_ip}"
        elif status == pi_setup.S_WAITING_DEVICE:
            pi_line = "\nLinux ethernet: interface ready — waiting for old computer"
        elif status == pi_setup.S_WAITING_CABLE:
            pi_line = "\nLinux ethernet: waiting for ethernet cable"
        elif status not in (pi_setup.S_NOT_LINUX, ""):
            pi_line = f"\nLinux ethernet: {status}"
    except Exception:
        pass

    body = (
        f"Rewind-Proxy [headless]  {network_note}\n"
        f"Time        : {now}\n"
        f"Proxy host  : {proxy_host}\n"
        f"Proxy port  : {proxy_port}\n"
        f"{pi_line}\n"
        "\n"
        "Configure your browser's HTTP proxy settings to point here,\n"
        "then browse any http:// URL — it will be served from the archive.\n"
    ).lstrip("\n")
...
class RewindProxyHandler(BaseHTTPRequestHandler):
    ...
    def log_message(self, fmt, *args):
        # Browsers request this automatically. It is handled locally and does
        # not need to clutter the terminal log.
    ...
    def _handle_direct(self, method: str, path: str):
        ...
        headless = os.environ.get("REWIND_HEADLESS", "0") == "1"

        # Browsers request /favicon.ico automatically. Return an empty,
        # cacheable response instead of a noisy 404.
        if path == "/favicon.ico":
            self._send_response(
                204,
                "No Content",
        ...
        # Root → home page (normal) or plain-text status (headless)
        if path in ("/", ""):
            if headless:
                self._send_response(200, "OK", b"text/plain", _build_headless_status())
            else:
                self._send_response(200, "OK", b"text/html", _build_home_page())
            return

        # /setup — human-readable setup page (disabled in headless mode)
        if path.startswith("/setup"):
            if headless:
                self._send_response(
                    200, "OK", b"text/plain",
                    b"Rewind-Proxy is running in headless mode.\n"
                    b"Configure your browser's HTTP proxy settings to:\n"
                    b"  Host: " + os.environ.get("REWIND_PROXY_HOST", "localhost").encode() + b"\n"
                    b"  Port: " + os.environ.get("REWIND_PROXY_PORT", "8080").encode() + b"\n",
                )
            else:
